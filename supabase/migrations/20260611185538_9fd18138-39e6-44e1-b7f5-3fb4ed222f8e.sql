...
BEGIN
  ...
  -- Resolve personalization tags using the recipient of this token (if any).
  SELECT NULLIF(TRIM(first_name), '') INTO v_first_name
  FROM personalized_deliveries
  WHERE experience_token = p_token
  LIMIT 1;

  v_message   := v_exp.message_content;
  v_cta_title := v_exp.cta_title;

  IF v_message IS NOT NULL THEN
    v_message := REPLACE(v_message, '{first_name}', COALESCE(v_first_name, ''));
  END IF;
  IF v_cta_title IS NOT NULL THEN
    v_cta_title := REPLACE(v_cta_title, '{first_name}', COALESCE(v_first_name, ''));
  END IF;
